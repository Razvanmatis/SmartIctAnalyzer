...
namespace TestCoverage.Implementations
{
    public class TestCoverageDeterminer : ITestCoverageDeterminer
    {
        ...
        public TestCoverageDeterminer(IEventService eventService, ILogger logger)
        {
            this.eventService = eventService;
            this.logger = logger;
        }

        public static string ToggleItem(string value)
        {
        ...
        public TestCoverageItems RunTheTestCoverageForDeterminingObjects(
            ...
            ref IList<IPCBComponent> pullDowns)
        {
            if (nets != null && nets.Count > 0)
            {
                ...
                ics = DefineIcs();
                pullUps = DefinePullUpResistors();
                pullDowns = DefinePullDownResistors();
                ...
                items.ItemTestCoverageObjects = TestCoverageDeterminer.ITEMSELECTED;
                if (mapObjects.Count > 0)
                {
                    items.TestCoverageMenuItemsEnabled = true;
                }

                CheckDoubleMatching(pullDowns, pullUps);
                LogResults();
            }
            else
            {
                items.ItemTestCoverageObjects = TestCoverageDeterminer.ITEMNOTSELECTED;
            }

            return items;
        }

        public async Task<TestCoverageItems> TestCoverageForIcs(TestCoverageItems items)
        {
            bool value = false;
        ...
        public async Task<TestCoverageItems> TestCoverageForPullUps(TestCoverageItems items)
        {
            bool value = false;
            IList<ITestCoverageResult> results = await DefineTestCoverageForPullUpDownObjects(DefineIcs(), DefinePullUpResistors(), false).ConfigureAwait(true);
            if (results.Count > 0)
            {
                eventService.Publish<RefreshTestcoverageResultObjectsEvent>(new RefreshTestcoverageResultObjectsEvent());
        ...
        public async Task<TestCoverageItems> TestCoverageForPullDowns(TestCoverageItems items)
        {
            bool value = false;
        ...
        public async Task<TestCoverageItems> TestCoverageForPullUpsDowns(TestCoverageItems items)
        {
            bool value = false;
        ...
        public async Task<TestCoverageItems> TestCoverageForAllOthers(TestCoverageItems items)
        {
        ...
        public async Task<TestCoverageItems> GetCompleteTestCoverageOfAllJtags(TestCoverageItems items)
        {
            bool value = false;
