...
As shown in \ref{fig:Tooloverview} the tool itself consits of seven different areas of interest:
\begin{enumerate}
...
\subsubsection{PCB Investigator API settings}

\begin{center}
\includegraphics[width=1.00\textwidth]{figures/attachments/pcbInvestigatorSettings.png}
	\begin{figure}[!h]
	\caption{PCB Investigator API settings}
	\label{fig:pcbInvestigatorSettings}
	\end{figure}
\end{center}

As shown in \ref{fig:pcbInvestigatorSettings} there is the possibility of changing the steps which should be considered for read out from the API. -1 means all steps should be read out. Other values could be defined semicolon separated

\subsubsection{GRPC server API connection settings}

\begin{center}
\includegraphics[width=1.00\textwidth]{figures/attachments/grpcServerSettings.png}
	\begin{figure}[!h]
	\caption{GRPC server API connection settings}
	\label{fig:grpcServerSettings}
	\end{figure}
\end{center}

As shown in \ref{fig:grpcServerSettings} there is the possibilty of changing the ip address of the GRPC server to connect to
...
As shown in \ref{fig:testCoverageSettings} there are the following settings available (blacklist and identifier):
\begin{itemize}
	\item Ground
	\item JTAG
	\item Power
\end{itemize}
Further more there is the possibility of defining the JTAG pin identifier for making sure being able to use not just the default values \textbf{TMS, TDI, TDO, TCK}. That means in detail for correctly determining a JTAG device there should be the following requirements accomplished:
\begin{enumerate}
	\item Set up the \textbf{IC identifier} setting from general settings in that way that the JTAG device is being recognized correctly as an IC component
	\item Set up the \textbf{JTAG nets identifier} from the test coverage settings in that way that the JTAG device is correctly being recognized within a corresponding net
	\item Set up the \textbf{JTAG pin identifier} setting from the test coverage settings in that way that the JTAG device is correctly being recognized by its pins
\end{enumerate}
...
\begin{itemize}
	\item Open ODB project folder - after choosing that action a new dialog appears to select the folder which should be transferred to the GRPC server as zipped content for read out by using the PCB-Investigator API
	\item Import BOM data file - After any project was successfully loaded then the import of a valid bill of materials file will be available. After choosing this a new window appears where to set up all relevant settings for import
	\item Import data from JSON file - a new dialog appears where you are able to select any JSON formatted file containing previously exported data of this application
	\item Export data into JSON file - after any project was loaded successfully then this item will be accessible. After choosing it a new dialog appears where you could select the target for creation of a valid JSON formatted file containing all data
\end{itemize}
...
\includegraphics[scale=0.60]{figures/attachments/selectBom.png}
	\begin{figure}[!h]
	\caption{Select BOM file dialog}
	\label{fig:selectBom}
	\end{figure}
\end{center}
...
\begin{itemize}
	...
	\item Path to BOM file - could be set by clicking on Select BOM file where then a new dialog for selecting any file will be opened
\end{itemize}
...
\begin{itemize}
	...
	\item Determine test coverage for JTAGs
...
If you click on Test coverage -> Boundary scan test -> Generate SVF data files then first a dialog will be shown to select the target folder where to save the created files at. This folder will only be considered if you are not working in the project file mode. If so the files will be automatically been saved within the project file. Afterward you will be asked for selecting the accoring BSDL file for all JTAG devices found. After you selected the BSDL file you will be informed about to make sure that the programmer is correctly connected to the JTAG device as the default vector first needs to be read out:
...
After you confirmed the dialog \ref{fig:makeSureToConnect} then the read out will be performed and the according SVF files for playing the pull down and pull up resistors will be generated.
...
You are further able to drag and drop directly files or folders to the application area. If something valid was dropped then it automatically should be opended or imported and if project mode is enabled saved further within the project file
