...
namespace PcbInvestigatorApiV10.Interfaces
{
    public abstract class AbstractDataProvider
    {
        public abstract IGrpcResult GetConvertedObjects(
        ...
        protected static IGeometricAttributes GetComponentGeometricAttributes(InterfaceCMPObject component)
        {
            Rectangle bounds = new Rectangle(
                (int)component.GetBoundsD().X,
                (int)component.GetBoundsD().Y,
                (int)component.GetBoundsD().Width,
                (int)component.GetBoundsD().Height);
            PointF centerPoint = new PointF((float)component.CenterPoint.X, (float)component.CenterPoint.Y);
        ...
        {
            RectangleF rectangleF = pinToCheck.GetBoundsD((ICMPObject)component).ToRectangleF();
            PointF centerPoint = pinToCheck.GetIPinPositionD((ICMPObject)component).ToPointF();
            Rectangle bounds = new Rectangle(
                (int)rectangleF.X,
                (int)rectangleF.Y,
        ...
        /// /// <param name="stepNo">the total step amount</param>
        /// <returns>the functional attributes</returns>
        protected static IFunctionalAttributes GetFunctionalAttributes(
            InterfaceCMPObject component, Func<IFunctionalAttributes, bool> isTestPoint, int stepNo)
        {
            ...
            string layerName = "layer_No: " + stepNo;
            string packageName = component.UsedPackageName;
            string normalizedName = string.Empty;
            return new FunctionalAttributes(
                componentType, reference, partName, layerName, packageName, normalizedName, isTestPoint, stepNo);
        }
