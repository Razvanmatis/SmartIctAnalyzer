...
HelpLibraryManagerLauncher.exe /product "VS" /version "400" /locale en-us /uninstall /silent /vendor "Vendor Name" /productName "PCB-Investigator" /mediaBookList "PCB-Investigator Interface Documentation" > NUL
...
REM build time.
IF EXIST "PCBI-Documentation.msha" COPY /Y "PCBI-Documentation.msha" HelpContentSetup.msha

REM Install the new content.
HelpLibraryManagerLauncher.exe /product "VS" /version "400" /locale en-us /sourceMedia "%CD%\HelpContentSetup.msha"
...
REM Uninstall first in case it is already there.  If not, it won't install below.  We'll ignore any error output
REM by redirecting it to NUL.
HelpLibraryManagerLauncher.exe /viewerVersion 2.0 /catalogName VisualStudio11 /locale en-us /wait 0 /operation uninstall /vendor "Vendor Name" /productName "PCB-Investigator" /bookList "PCB-Investigator Interface Documentation" > NUL

REM Install the new content.
HelpLibraryManagerLauncher.exe /viewerVersion 2.0 /catalogName VisualStudio11 /locale en-us /wait 0 /operation install /sourceUri "%CD%\PCBI-Documentation.msha"
